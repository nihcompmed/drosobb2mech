...
if fillingmethod == 'zero':
    data_fn = "all_ctgxyz_99genes_fillzero_fillzero_t012345_90percent_and_shuffled_tellnextg.csv"#all_tgvxyz_t012345_train5000cells_250507_shuffled.csv"
elif fillingmethod == 'avg':
    data_fn = "all_ctgxyz_99genes_fillavg_fillzero_t012345_90percent_and_shuffled_tellnextg.csv"#all_ctgxyz_99g_fillavg_t012345_train5000cells_shuffled.csv"
...
# Create a 32820 x 27 matrix of ones
mask_array = np.ones((32820, 27), dtype=int)  # int type, like CSV read usually gives
...
# build boolean mask per-gene for CURRENT time (drop KOs to align with kept genes)
...
class VAE(pl.LightningModule):
    ...
    def _jacobian_penalty(self, z_mu):
        ...
        z0 = z_mu.detach()                    # prevent encoder grads
    ...
    def _directional_penalty(self, mu_t, g_t, g_next, mask_bool, has_next, xyz):
        """
        Encourage J_dec(mu_t) to map latent motion towards observed Δg direction.
        Cosine alignment against Δg_t = g_next - g_t (masked). (5) applies a small
        threshold on ‖Δg‖ to avoid pushing on near-static pairs.
        """
        ...
        mu_next = self._encode_mu(x_next)           # detached
        mu_curr = mu_t.detach()
        dmu = mu_next - mu_curr
        ...
        # (5) rows usable: has_next & ‖Δg‖>dg_thresh
    ...
    def training_step(self, batch, batch_idx):
        ...
        # penalties
        jac_pen = self._jacobian_penalty(mu)                       # cusp smoother
        dir_pen = self._directional_penalty(mu, g, g_n, mask, has_next, xyz)  # (5)

        # ---- NEW: late-stage ramp for Jacobian penalty (env-controlled) ----
        ramp_mult  = float(os.getenv("JAC_RAMP_MULT", "1.0"))                 # e.g., 2.5
        ramp_start = int(os.getenv("JAC_RAMP_START", str(max_epochs // 2)))   # default mid-training
        ramp_end   = int(os.getenv("JAC_RAMP_END",   str(int(max_epochs * 0.9))))
