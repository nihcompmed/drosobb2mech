#!/usr/bin/env python3
# calibrate_alpha_noGamma_pruned.py

"""
Alpha calibration for Hill model CSVs without gamma (b0, V, K, n only).
Prunes couplings: if |Vij| < 1e-3, then Vij, Kij, nij = 0.

Suggest run:
python Step7p5_SecPerturb_alphacalibration.py \
  --code_csv HillModels/HillLassoNoPrune_250815001_a1em02_l1em05_epoch=135-val_mse=0.02318_params_298x99.csv \
...
class HillField(nn.Module):
    def __init__(self, b0, V, K, n):
        super().__init__()
        self.G = V.shape[0]
        self.b0 = nn.Parameter(torch.as_tensor(b0, dtype=torch.float64, device=DEVICE), requires_grad=False)
        self.V  = nn.Parameter(torch.as_tensor(V,  dtype=torch.float64, device=DEVICE), requires_grad=False)
        self.K  = nn.Parameter(torch.as_tensor(K,  dtype=torch.float64, device=DEVICE), requires_grad=False)
        self.n  = nn.Parameter(torch.as_tensor(n,  dtype=torch.float64, device=DEVICE), requires_grad=False)
        # only train α
        self.log_alpha = nn.Parameter(torch.zeros(1, dtype=torch.float64, device=DEVICE), requires_grad=True)
...
# -------------------- CSV helpers --------------------
def load_hill_csv(path, prune_thresh=1e-3):
    ...
    # ---- Prune small couplings ----
    mask = np.abs(V) < prune_thresh
    V[mask] = 0.0
    K[mask] = 0.0
    n[mask] = 0.0
...
def main():
    ...
    ap.add_argument("--prune_thresh", type=float, default=1e-3,
                    help="threshold for |Vij| pruning before calibration")
    args = ap.parse_args()

    b0, V, K, n, G = load_hill_csv(args.code_csv, prune_thresh=args.prune_thresh)
    ...
    alpha = torch.exp(field.log_alpha).item()
    print(f"==> learned alpha = {alpha:.6f}")

    # bake α into b0 and V
    b0_new = alpha * b0
    V_new  = alpha * V
    out_path = os.path.splitext(args.code_csv)[0] + "_alphaCal.csv"
    save_hill_csv(out_path, b0_new, V_new, K, n)
