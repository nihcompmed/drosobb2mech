...
Options:
  --deriv_only   If set, only derivative + L1(V) losses are used (skip state/collocation).
"""

import glob, re, os, sys, importlib
import numpy as np
import pandas as pd
...
def build_pushfwd_from_vae(imputed_csv: str,
    ...
    for i in range(0, len(pairs), bsize_pairs):
        ...
        s_grid = torch.linspace(0.0, 1.0, n_sub+1, dtype=torch.float64, device=DEVICE)
        z_stack = torch.stack([z_t + s * dz for s in s_grid], dim=0)

        g_stack = torch.stack([decode(z_stack[k]) for k in range(n_sub+1)], dim=0)
        ...
        gdots = torch.stack([jvp_batch(z_stack[k], dz) for k in range(n_sub+1)], dim=0)
...
class HillTrainerTorch:
    def __init__(self, G, max_lr, ep_maxlr, max_epochs, l1_lambda,
                 ...
                 sigma_vec=None, wd=0.0, deriv_only=False):
    ...
    def _weights(self, epoch):
        ...
        if epoch <= e0: return 1.0, 0.1, 0.3
        ...
        for epoch in range(self.max_epochs):
            ...
            for batch in train_loader:
                ...
                if self.deriv_only:
                    ...
                    loss_coll  = torch.tensor(0.0, device=DEVICE)
                else:
                ...
                if self.deriv_only:
                    loss = w_deriv*loss_deriv + l1_pen
                else:
            ...
            with torch.no_grad():
                ...
                monitor = v_deriv if self.deriv_only else (0.3*v_deriv + 0.7*v_state)
...
def main():
    ...
    G = out_dim
    if G <= 0:
        raise RuntimeError("Could not infer VAE decoder output_dim (G).")
